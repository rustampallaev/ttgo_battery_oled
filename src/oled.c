...
void ssd1366_init() {
	...
	if (err != ESP_OK) {
		printf("display cfg failed. code: 0x%.2X", err);
	}
	i2c_cmd_link_delete(cmd);
}

void ssd1366_display_pattern(void) {
...
void ssd1366_display_clear(void) {
...
void ssd1366_scroll(void) {
	...
	err = i2c_master_cmd_begin(I2C_NUM_0, cmd, 10/portTICK_PERIOD_MS);
	if (err != ESP_OK) {
		printf("scroll command failed. code: 0x%.2X", err);
	}

	i2c_cmd_link_delete(cmd);
}

void ssd1366_display_text(char *text) {
...
void oled_put_text(char *text) {
	ssd1366_display_text(text);
}

void oled_clear(void) {
	ssd1366_display_clear();
}